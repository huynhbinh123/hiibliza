<template>
  <div>
    ...
    >
      <MainTitleAll title="Music" />
      <SelectBar
        :tabs="['All', 'Audio', 'Video', 'Playlist']"
        @update="onTabChange"
      />

      <div
        class="grid lg:grid-cols-3 grid-cols-1 gap-4 w-full mx-auto place-items-center"
      >
        <!-- music -->
        <MusicItem
...
const filteredAll = computed(() => {
  if (selectedTab.value === "All") return allMusic;
  return allMusic.filter((item) => item.title === selectedTab.value);
});
